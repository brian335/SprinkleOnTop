...
function Card({
  ...
  return (
    ...
    >
      <div className="h-full w-full overflow-hidden rounded-[1.1rem] border-2 border-ink/80 bg-cream-deep">
        <img
          src={cake.image}
          alt={cake.alt}
          draggable={false}
 ...
 * seconds the top one is dealt away to the side with a burst of sprinkles.
 * You can also throw it yourself: the top card is draggable, which is the bit
 * that makes people play rather than scroll straight past.
 *
...
export function CakeStack({ onAccent }: { onAccent?: (hex: string) => void }) {
  ...
  return (
    ...
    >
      ...
      {/* which cake you are on, and a way to jump */}
      ...
      <span className="absolute -bottom-[4.4rem] left-1/2 -translate-x-1/2 font-hand text-lg whitespace-nowrap text-ink-faint">
        drag a cake away to see the next one
      </span>
    </div>
  )
}
